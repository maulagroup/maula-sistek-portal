          ...
          {isLoading ? (
            ...
            <div className="space-y-0">
              {activityLogs.map((log, index) => (
                <div key={log.id} className={`flex gap-4 py-6 ${index < activityLogs.length - 1 ? 'border-b border-slate-800' : ''}`}>
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <Badge variant="outline" className="font-medium border-slate-700">
                        {log.dibuat_oleh}
                      </Badge>
                      {log.projects && (
                        <Badge variant="secondary" className="flex items-center gap-1 bg-slate-900 border-slate-800">
                          <FolderKanban className="h-3 w-3" />
                          {log.projects.nama_project}
                        </Badge>
                      )}
                      {log.projects?.clients && (
                        <Badge variant="outline" className="flex items-center gap-1 border-slate-700">
                    ...
                    <p className="text-slate-200">{log.pesan}</p>
                  </div>
                </div>
              ))}
            </div>
