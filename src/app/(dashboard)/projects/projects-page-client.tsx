...
function ProjectForm({
  ...
  return (
    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto border-slate-800 bg-slate-950">
      <DialogHeader>
...
const EmptyState = ({
  ...
  <div className="flex flex-col items-center justify-center py-12 text-center">
    ...
    <h3 className="text-xl font-semibold mb-2 text-slate-200">Belum ada project</h3>
              ...
              <TableBody>
                {projects.map((project) => (
                  <TableRow key={project.id} className="border-slate-800">
                    <TableCell className="font-medium text-slate-200">{project.nama_project}</TableCell>
                    <TableCell className="text-slate-300">{project.client_id}</TableCell>
                    <TableCell className="text-slate-300">{project.jenis_layanan}</TableCell>
                    <TableCell>
                      <Badge className={STATUS_COLORS[project.status] || "bg-gray-900/30 text-gray-400 border-gray-800"} variant="outline">
                        {project.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-slate-300">{project.deadline ? new Date(project.deadline).toLocaleDateString("id-ID") : "-"}</TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        <Link href={`/projects/${project.id}`} className="flex h-10 w-10 items-center justify-center rounded-md hover:bg-slate-800">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="border-slate-800 bg-slate-950">
                            ...
                            <AlertDialogFooter>
                              <AlertDialogCancel className="border-slate-700">Batal</AlertDialogCancel>
                              <AlertDialogAction
